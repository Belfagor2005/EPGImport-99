...
from os import exists, getsize, join, path as ospath, statvfs, symlink, unlink, access as osaccess, W_OK
...
class EPGImport:
        ...
        if dirname in ServerStatusList:
            ...
            if not dlderror:
                ...
                try:
                    ...
                    response.close()
                    return ServerStatusList[dirname]
                # Calcolo della differenza di giorni
    ...
    def urlDownload(self, sourcefile, afterDownload, downloadFail):
        ...
        # Debug: Verifica tipi prima di chiamare downloadPage
        ...
        if self.source.nocheck == 1:
            print("[EPGImport] Not checking the server since nocheck is set for it: " + sourcefile)
