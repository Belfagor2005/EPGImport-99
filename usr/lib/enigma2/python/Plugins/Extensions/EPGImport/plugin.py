...
def lastMACbyte():
    try:
        return int(
            open("/sys/class/net/eth0/address").readline().strip()[-2:], 16)
...
config.plugins.epgimport.repeat_import = ConfigInteger(
    default=0, limits=(0, 23))
...
config.plugins.epgimport.pathdb = ConfigDirectory(
    default='/etc/enigma2/epg.dat')
...
config.plugins.epgimport.deepstandby_afterimport = NoSave(
...
config.plugins.epgimport.day_profile = NoSave(
    ConfigSelection(choices=[("1", _("Press OK"))], default="1"))
...
for i in range(7):
    config.plugins.extra_epgimport.day_import[i] = ConfigEnableDisable(
        default=True)
                            ...
                            while True:
                                ...
                                if not (service.flags & mask):
                                    if service.flags & alternative:
                                        alternative_list = getAlternatives(
                                            service)
                                        if alternative_list:
...
def channelFilter(ref):
    if not ref:
        return False
    # Ignore non IPTV
    if config.plugins.epgimport.import_onlyiptv.value and (
            "%3a//" not in ref.lower() or ref.startswith("1")):
        return False
    ...
    if serviceIgnoreList is None:
        serviceIgnoreList = [
            getRefNum(x) for x in filtersServices.filtersServicesList.servicesList()]
...
def startImport():
    if not epgimport.isImportRunning():
        ...
        if config.plugins.epgimport.clear_oldepg.value and hasattr(
        ...
        epgimport.beginImport(
            longDescUntil=config.plugins.epgimport.longDescDays.value *
            24 *
            3600 +
            time())
...
class EPGImportConfig(ConfigListScreen, Screen):
    ...
    def initConfig(self):
        ...
        self.cfg_enabled = getConfigListEntry(_("Automatic import EPG"), self.EPG.enabled, _(
            "When enabled, it allows you to schedule an automatic EPG update at the given days and time."))
        self.cfg_pathdb = getConfigListEntry(
            _("Path DB EPG"),
            self.EPG.pathdb,
            _("Specify the path folder for save EPG.dat file."))
        self.cfg_source_import = getConfigListEntry(
            _("Source EPG"), self.EPG.extra_source, _("Specify the source for EPG."))
        self.cfg_wakeup = getConfigListEntry(
            dx + _("Automatic start time"),
            self.EPG.wakeup,
            _("Specify the time for the automatic EPG update."))
        self.cfg_deepstandby = getConfigListEntry(
            dx + _("When in deep standby"),
            self.EPG.deepstandby,
            _("Choose the action to perform when the box is in deep standby and the automatic EPG update should normally start."))
        self.cfg_day_profile = getConfigListEntry(
            dx + _("Choice days for start import"),
            self.EPG.day_profile,
            _("You can select the day(s) when the EPG update must be performed."))
        self.cfg_shutdown = getConfigListEntry(
            dx + _("Return to deep standby after import"),
            self.EPG.shutdown,
            _("This will turn back waked up box into deep-standby after automatic EPG import."))
        self.cfg_standby_afterwakeup = getConfigListEntry(
            dx + _("Standby at startup"),
            self.EPG.standby_afterwakeup,
            _("The waked up box will be turned into standby after automatic EPG import wake up."))
        self.cfg_repeat_import = getConfigListEntry(
            dx + _("Hours after which the import is repeated"),
            self.EPG.repeat_import,
            _("Number of hours (1-23, or 0 for no repeat) after which the import is repeated. This value is not saved and will be reset when the GUI restarts."))
        self.cfg_runboot = getConfigListEntry(_("Start import after booting up"), self.EPG.runboot, _(
            "Specify in which case the EPG must be automatically updated after the box has booted."))
        self.cfg_runboot_day = getConfigListEntry(
            dx + _("Consider setting \"Days Profile\""),
            self.EPG.runboot_day,
            _("When you decide to import the EPG after the box booted mention if the \"days profile\" must be take into consideration or not."))
        self.cfg_runboot_restart = getConfigListEntry(
            dx + _("Skip import on restart GUI"),
            self.EPG.runboot_restart,
            _("When you restart the GUI you can decide to skip or not the EPG data import."))
        self.cfg_run_after_standby = getConfigListEntry(
            _("Start import after standby"),
            self.EPG.run_after_standby,
            _("Start import after resuming from standby mode."))
        self.cfg_import_onlybouquet = getConfigListEntry(
            _("Load EPG only services in bouquets"),
            self.EPG.import_onlybouquet,
            _("To save memory you can decide to only load EPG data for the services that you have in your bouquet files."))
        self.cfg_import_onlyiptv = getConfigListEntry(
            _("Load EPG only for IPTV channels"),
            self.EPG.import_onlyiptv,
            _("To save memory you can decide to load EPG data only  for theIPTV channels."))
        self.cfg_showinextensions = getConfigListEntry(_("Show \"EPGimport now\" in extensions"), self.EPG.showinextensions, _(
            "Display a shortcut \"EPG import now\" in the extension menu. This menu entry will immediately start the EPG update process when selected."))
        self.cfg_showinmainmenu = getConfigListEntry(_("Show \"EPGimport\" in epg menu"), self.EPG.showinmainmenu, _(
            "Display a shortcut \"EPG import\" in your STB epg menu screen. This allows you to access the configuration."))
        self.cfg_longDescDays = getConfigListEntry(_("Load long descriptions up to X days"), self.EPG.longDescDays, _(
            "Define the number of days that you want to get the full EPG data, reducing this number can help you to save memory usage on your box. But you are also limited with the EPG provider available data. You will not have 15 days EPG if it only provide 7 days data."))
        self.cfg_parse_autotimer = getConfigListEntry(_("Run AutoTimer after import"), self.EPG.parse_autotimer, _(
            "You can start automatically the plugin AutoTimer after the EPG data update to have it refreshing its scheduling after EPG data refresh."))
        self.cfg_clear_oldepg = getConfigListEntry(_("Delete current EPG before import"), config.plugins.epgimport.clear_oldepg, _(
            "This will clear the current EPG data in memory before updating the EPG data. This allows you to always have a clean new EPG with the latest EPG data, for example in case of program changes between refresh, otherwise EPG data are cumulative."))
        self.cfg_filter_custom_channel = getConfigListEntry(_("Also apply \"channel id\" filtering on custom.channels.xml"), self.EPG.filter_custom_channel, _(
            "This is for advanced users that are using the channel id filtering feature. If enabled, the filter rules defined into /etc/epgimport/channel_id_filter.conf will also be applied on your /etc/epgimport/custom.channels.xml file."))
        self.cfg_execute_shell = getConfigListEntry(_("Execute shell command before import EPG"), self.EPG.execute_shell, _(
            "When enabled, then you can run the desired script before starting the import, after which the import of the EPG will begin."))
        self.cfg_shell_name = getConfigListEntry(
            dx + _("Shell command name"),
            self.EPG.shell_name,
            _("Enter shell command name."))
    ...
    def createSetup(self):
        ...
        if fileExists(AUTOTIMER_PLUGIN_PATH):
            try:
                ...
                print(
                    "[EPGImport] AutoTimer plugin not installed correctly",
                    file=log)
    ...
    def newConfig(self):
        ...
        if cur in (
                self.cfg_enabled,
                self.cfg_shutdown,
                self.cfg_deepstandby,
                self.cfg_runboot,
                self.cfg_import_onlyiptv,
                self.cfg_execute_shell):
            self.createSetup()
        self.setInfo()
    ...
    def keyGreen(self):
        ...
        if self.EPG.parse_autotimer.value and not fileExists(
                AUTOTIMER_PLUGIN_PATH):
        ...
        if self.prev_onlybouquet != config.plugins.epgimport.import_onlybouquet.value or (
                autoStartTimer is not None and autoStartTimer.prev_multibouquet != config.usage.multibouquet.value):
    ...
    def keyOk(self):
        ...
        if sel == self.EPG.pathdb:
            ...
            self.session.openWithCallback(
                ...
                VirtualKeyBoard,
                text=self.EPG.shell_name.value)
        else:
            return
    ...
    def openDirectoryBrowser(self, path, itemcfg):
        try:
            callback_map = {
                "pathdb": self.openDirectoryBrowserCB(
    ...
    def updateStatus(self):
        ...
        if isFilterRunning == 1:
            ...
            text = self.importStatusTemplate % (
                src.description, epgimport.eventCount)
            self["status"].setText(text)
            return
        ...
        if lastImportResult and (lastImportResult != self.lastImportResult):
            ...
            try:
                if isinstance(start, str):
                    ...
                    raise ValueError(
                        "Start value is not a valid timestamp or string")
                ...
                try:
                    ...
                    print(
                        "[EPGImport] Fallback with FuzzyTime also failed:", e)
            self["statusbar"].setText(
                _("Last import: %s %s, %d events") %
                (d, t, count))
    ...
    def doimport(self, one_source=None):
        ...
        if config.plugins.epgimport.import_onlybouquet.isChanged() or (
                autoStartTimer is not None and autoStartTimer.prev_multibouquet != config.usage.multibouquet.value):
            EPGConfig.channelCache = {}
        if one_source is None:
            ...
            cfg = one_source
        sources = [
            s for s in EPGConfig.enumSources(
                CONFIG_PATH,
                filter=cfg["sources"])]
        if not sources:
    ...
    def do_import_callback(self, confirmed):
        ...
        try:
            if config.plugins.epgimport.execute_shell.value and config.plugins.epgimport.shell_name.value:
                Console().eBatch(
                    [config.plugins.epgimport.shell_name.value], self.executeShellEnd, debug=True)
            else:
                startImport()
        except Exception as e:
    ...
    def openMenu(self):
        menu = [(_("Show log"), self.showLog),
                (_("Ignore services list"), self.openIgnoreList)]
        ...
        def setAction(choice):
            if choice:
                choice[1]()
        self.session.openWithCallback(
            setAction, ChoiceBox, title=text, list=menu)
...
class EPGImportSources(Screen):
    ...
    def __init__(self, session):
        ...
        self.tree = []
        self.giturl = SOURCE_LINKS.get(
            config.plugins.epgimport.extra_source.value)
        cfg = EPGConfig.loadUserSettings()
        filter = cfg["sources"]
        cat = None
        for x in EPGConfig.enumSources(
                CONFIG_PATH, filter=None, categories=True):
        ...
        self["list"] = ExpandableSelectionList.ExpandableSelectionList(
            self.tree, enableWrapAround=True)
...
class EPGImportProfile(ConfigListScreen, Screen):
    ...
    def __init__(self, session):
        ...
        for i in range(7):
            settingsList.append(
                getConfigListEntry(
                    weekdays[i],
                    config.plugins.extra_epgimport.day_import[i]))
    ...
    def save(self):
        if all(
                not config.plugins.extra_epgimport.day_import[i].value for i in range(7)):
            msg = _(
...
class EPGImportLog(Screen):
    ...
    def save(self):
        try:
            ...
            self["list"].setText(
                "Failed to write /tmp/epgimport.log:str" + str(e))
        self.close(True)
...
def doneImport(reboot=False, epgfile=None):
    ...
    try:
        ...
        print(
            "[EPGImport] Error to save last import date and count event",
            file=log)
    ...
    if reboot:
        ...
        if config.plugins.epgimport.parse_autotimer.value and fileExists(
                AUTOTIMER_PLUGIN_PATH):
            try:
    ...
    def __init__(self, session, parse=False):
        ...
        if config.plugins.epgimport.enabled.value:
            if parse:
                ...
                print(
                    "[EPGImport] Wait for parse autotimers 600 sec.",
                    file=log)
            else:
    ...
    def runCheckDeepstandby(self):
        ...
        if config.plugins.epgimport.shutdown.value and config.plugins.epgimport.deepstandby.value == "wakeup":
            if config.plugins.epgimport.deepstandby_afterimport.value and getFPWasTimerWakeup():
                ...
                if Screens.Standby.inStandby and not self.session.nav.getRecordings(
                ) and not Screens.Standby.inTryQuitMainloop:
                    print(
                        "[EPGImport] Returning to deep standby after wake up for import",
                        file=log)
                    self.session.open(Screens.Standby.TryQuitMainloop, 1)
                else:
                    print(
                        "[EPGImport] No return to deep standby, not standby or running recording",
                        file=log)
...
class AutoStartTimer:
    def __init__(self, session):
        ...
        self.pauseAfterFinishImportCheck.callback.append(
            self.afterFinishImportCheck)
        self.pauseAfterFinishImportCheck.startLongTimer(30)
        self.container = None
        config.misc.standbyCounter.addNotifier(
            self.standbyCounterChangedRunImport)
        self.update()
    ...
    def update(self, atLeast=0, clock=False):
        ...
        wake_str = strftime("%Y-%m-%d %H:%M:%S",
                            localtime(wake)) if wake > 0 else "Not set"
        print(
            "[EPGImport] WakeUpTime now set to",
            wake_str,
            "(now=%s)" %
            now_str,
            file=log)
    ...
    def onTimer(self):
        ...
        if wake - now < 60:
            ...
            if repeat_time:
                ...
                print(
                    "[EPGImport] start once repeat timer, wait in nours -",
                    repeat_time,
                    file=log)
            atLeast = 60
        self.update(atLeast)
                ...
                if 0 < wake - now <= 60 * 5:
                    if config.plugins.epgimport.standby_afterwakeup.value:
                        if not Screens.Standby.inStandby:
                            ...
                            print(
                                "[EPGImport] Run to standby after wake up", file=log)
                    if config.plugins.epgimport.shutdown.value:
                        if not config.plugins.epgimport.standby_afterwakeup.value:
                            if not Screens.Standby.inStandby:
                                Notifications.AddNotification(
                                    Screens.Standby.Standby)
                                print(
                                    "[EPGImport] Run to standby after wake up for checking", file=log)
                        if not config.plugins.epgimport.deepstandby_afterimport.value:
                            ...
                            print(
                                "[EPGImport] start wait_timer (10sec) for goto standby", file=log)
        ...
        if Screens.Standby.inStandby:
            try:
                if self.afterStandbyRunImport not in Screens.Standby.inStandby.onClose:
                    Screens.Standby.inStandby.onClose.append(
                        self.afterStandbyRunImport)
            except BaseException:
                print(
                    "[EPGImport] error inStandby.onClose append afterStandbyRunImport",
                    file=log)
    ...
    def startStandby(self):
        if Screens.Standby.inStandby:
            ...
            try:
                ...
                print(
                    "[EPGImport] error inStandby.onClose append .onLeaveStandby",
                    file=log)
    ...
    def onLeaveStandby(self):
        if config.plugins.epgimport.deepstandby_afterimport.value:
            config.plugins.epgimport.deepstandby_afterimport.value = False
            print(
                "[EPGImport] checking standby remove, not deep standby after import",
                file=log)
...
def WakeupDayOfWeek():
    ...
    if cur_day >= 0:
        for i in range(1, 8):
            if config.plugins.extra_epgimport.day_import[(
                    cur_day + i) % 7].value:
                return i
    return start_day
...
def onBootStartCheck():
    ...
    print("[EPGImport] now=%d wake=%d wake-now=%d" %
          (now, wake, wake - now), file=log)
    if (wake < 0) or (wake - now > 600):
        ...
        if config.plugins.epgimport.runboot_restart.value and runboot != "3":
            if exists(ANSWER_BOOT_FILE):
                on_start = False
                print(
                    "[EPGImport] not starting import - is restart enigma2",
                    file=log)
            else:
                try:
                    ...
                    print(
                        "Failed to create /tmp/.EPGImportAnswerBoot",
                        file=log)
        if config.plugins.epgimport.runboot_day.value:
            ...
            if not config.plugins.extra_epgimport.day_import[cur_day].value:
                on_start = False
                print(
                    "[EPGImport] wakeup day of week does not match",
                    file=log)
        if on_start:
            print(
                "[EPGImport] starting import because auto-run on boot is enabled",
                file=log)
            autoStartTimer.runImport()
    else:
        print(
            "[EPGImport] import to start in less than 10 minutes anyway, skipping...",
            file=log)
    ...
    if reason == 0 and _session is None:
        ...
        if exists(sourcesFile):
            try:
                ...
                print(f"[EPGImport] Error extract sources {e}", file=log)
...
config.plugins.epgimport.showinextensions.addNotifier(
    setExtensionsmenu, initial_call=False, immediate_feedback=False)
