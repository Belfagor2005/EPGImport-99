        ...
        for service in services:
            ...
            for event in events:
                ...
                for desc in longDescription:
                    # if longDescription[i][0] not in epg_event_description_dict:
                    # if not exist_event(longDescription[i][0]) :
                    if desc[0] not in self.EPGDAT_HASH_EVENT_MEMORY_CONTAINER:
                        # DESCRIPTION DATA
                        pack_2 = s_BB.pack(0x4e, len(desc[1])) + desc[1]
                        self.EPG_HEADER2_description_count += 1
                        # DESCRIPTION HEADER (2 int) will be computed at the end just before EPG.DAT write
                        ...
                        self.EPGDAT_HASH_EVENT_MEMORY_CONTAINER[desc[0]] = [pack_2, 1]
                    else:
                ...
                # pack_3 = s_BB.pack(0x01, 0x0a + EPG_EVENT_HEADER_datasize)
                ...
                pack_6 = s_BBB.pack(*TL_hexconv(event_duration_HMS))  # LENGTH
                pack_7 = s_I.pack(shortDescription[0])  # REF DESC short (title)
    ...
    def final_process(self):
        if self.EPG_TOTAL_EVENTS > 0:
            ...
            for temp in sorted(list(self.EPGDAT_HASH_EVENT_MEMORY_CONTAINER.keys())):
                ...
                pack_4 = s_ii.pack(temp, temp_crc_data[1])
